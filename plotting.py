...
def midplane_comparison(phantom_dump, analytics_file_location):
    ...
    # applying exponential taper
    #A_rho *= np.exp(-np.sqrt(A_X**2 + A_Y**2)/100)
    ...
    A_int_vr = interp_v_r.ev(new_Y, new_X)
    A_int_vphi = interp_v_phi.ev(new_Y, new_X)
    A_int_rho = interp_v_rho.ev(new_Y, new_X)

    # find residuals
    
    R_vr = -1*(PD.vr_xy - A_int_vr)
    R_vphi = -1*(-1*PD.vphi_xy - -1*A_int_vphi )
    R_rho = np.abs(PD.rho_xy - A_int_rho)
    ...
    fig.colorbar(pcm_vphi, ax=ax[1, :], location='right', label='Azimuthal Velocity [km/s]')
    ...
    ax[0,2].set_title('Analytics - Simulation')
